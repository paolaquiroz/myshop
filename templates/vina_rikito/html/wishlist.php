	...
	if ($result > 0 ){
			...
			else
			{
				...
				else 
				{
					...
					$addtofavorites .= '<div class="addtofav_aws_icon_active jutooltip"  title="'.JText::_('VM_ALL_FAVORITE_PRODUCTS').'">';
					...
					$addtofavorites .= '<span>'.JText::_('VM_ALL_FAVORITE_PRODUCTS').'</span></div></a>';
				}
			}
	}
	if ($result == 0){
		if($product_id == $favorite_id && $mode == "fav_add") {
			...
			if ($guest_enabled || !$user->guest){
					...
					else $addtofavorites .= '"addtofav_aws_icon jutooltip"';
					...
					$addtofavorites .= '<span>'.JText::_('VM_ADD_TO_FAVORITES').'</span></button>
					...
					$addtofavorites .= '<span>'.JText::_('VM_ADD_TO_FAVORITES').'</span></button></a>';
			}
		}
	}
